...
def serial_thread():
	...
	while True:
		...
		print(f"[RS485] Torque: {torque} Nm | Speed: {speed} RPM")
...
def socket_thread():
	...
	print("[TCP] Waiting for effmea.exe...")
	...
	while True:
		...
		while len(rx_buffer) >= 12:
			...
			status = packet[0]
			error  = packet[1]
			tor    = struct.unpack(">h", packet[2:4])[0]
			vel    = struct.unpack(">i", packet[4:8])[0]
			pos    = struct.unpack(">i", packet[8:12])[0]

			print(f"[TCP RX] status={status} tor={tor} vel={vel} pos={pos}")
		...
		with sensor_lock:
			sensor_torque = latest_sensor_data["torque"]

		# 예: 센서 토크를 그대로 target으로 사용
		target_torque = int(sensor_torque)
		target_velocity = 0
		mode = 10  # CST 예시

		try:
			tx_packet = struct.pack("!hih", target_torque, target_velocity, mode)
			conn.send(tx_packet)
		except BlockingIOError:
			pass
...
if __name__ == "__main__":
	...
	t2 = threading.Thread(target=socket_thread, daemon=True)
