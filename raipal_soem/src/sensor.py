...
def read_sensor(ser):
    torque, speed_raw, crc = parse_frame(read_frame(ser))
    return torque, speed_raw, crc


    
def read_input_value():
    """Reads the latest input value written by simple_test.c"""
    try:
...
def monitor():
	print("Monitoring input and sensor values...")
	# RS485 포트 오픈
	ser = serial.rs485.RS485(
		port='COM7',         # 본인 환경에 맞는 포트
		baudrate=38400,
		bytesize=8,
		parity='N',
		stopbits=1,
	...
	ser.rs485_mode = serial.rs485.RS485Settings()   # DE/RE 자동제어 활성화
	print("Connected!")
			
	try:
		shift = sync_to_valid_frame(ser)
		...
		while True:
			...
			#print(frame)
			print(f"Input: {input_value:.3f}")
			print(f"Torque: {torque} Nm| Speed: {speed} RPM| CRC: {crc:#04x}")
			time.sleep(0.01)
	finally:
		ser.close()
